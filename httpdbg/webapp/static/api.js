...
function save_request(request_id, request, session_id) {
    ...
    if (initiator !== null) {  // the initiator may be missing if the clean list is executed in parrallel
        ...
        if (request.in_progress) {
            request.status_code_view = '<svg class="icon" alt="loading"><use href="#wait-sandclock-icon"></use></svg>';
        } else {
            switch (request.status_code) {
                case 0:
                    ...
                    break;
                case -1:
                    request.status_code_view = '<svg class="icon" alt="load failed"><use href="#math-multiplication-icon"></use></svg>';
                    break;
                default:
